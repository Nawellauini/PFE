...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Requête d'insertion du rapport - CORRIGÉE pour correspondre à la structure de la table
    $query = "INSERT INTO rapports_inspection (titre, id_classe, id_professeur, id_inspecteur, commentaires, recommandations, date_creation) 
              VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt === false) {
        ...
        header("Location: ajouter_rapport.php?error=" . urlencode("خطأ في إعداد الاستعلام: " . $conn->error));
        exit();
    }
    
    $stmt->bind_param("siiisss", $titre, $id_classe, $id_professeur, $id_inspecteur, $commentaires, $recommandations, $date_creation);
    ...
    if ($stmt->execute()) {
        ...
        if (isset($_FILES['fichiers']) && !empty($_FILES['fichiers']['name'][0])) {
            ...
            foreach ($_FILES['fichiers']['name'] as $key => $name) {
                if ($_FILES['fichiers']['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tmp_name, $destination)) {
                        ...
                        $stmt_file->bind_param("isss", $rapport_id, $original_name, $destination, $file_type);
            ...
            if (!$upload_success) {
                ...
                header("Location: liste_rapports.php?success=partial&error=" . urlencode($error_message));
                exit();
            }
        }
        
        // Redirection en cas de succès
        header("Location: liste_rapports.php?success=1");
        exit();
    } else {
        // Journaliser l'erreur pour le débogage
        error_log("خطأ في إدراج التقرير: " . $stmt->error);
        
        // Rediriger avec un message d'erreur
        header("Location: ajouter_rapport.php?error=" . urlencode("خطأ أثناء إضافة التقرير: " . $stmt->error));
        exit();
    }
}
?>
