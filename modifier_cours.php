...
if ($result->num_rows == 0) {
    ...
    header("Location: liste_cours.php?error=لا يمكنك تعديل هذا الدرس");
    exit();
}
    ...
    if ($result_check->num_rows == 0) {
        ...
        if (isset($_FILES['illustration']) && $_FILES['illustration']['error'] == 0) {
            ...
            if (in_array(strtolower($ext), $allowed)) {
                ...
                if (move_uploaded_file($_FILES['illustration']['tmp_name'], $illustration_path)) {
                    ...
                    $sql_update .= ", illustration = ?";
                    $params[] = $illustration_path;
                    $types .= "s";
                } else {
        ...
        if (!isset($error)) {
            ...
            if ($stmt_update->execute()) {
                ...
                header("Location: liste_cours.php?message=تم تعديل الدرس بنجاح");
                exit();
            } else {
    ...
    <div class="container">
        ...
        <div class="card animate-fadeInUp">
            ...
            <div class="card-body">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id_cours; ?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="titre" class="form-label">عنوان الدرس *</label>
                        <input type="text" id="titre" name="titre" class="form-control" value="<?php echo htmlspecialchars($cours['titre']); ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="description" class="form-label">وصف الدرس *</label>
                        <textarea id="description" name="description" class="form-control" required><?php echo htmlspecialchars($cours['description']); ?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($cours['fichier'])): ?>
                            <div class="current-file">
                                ...
                                <span>الملف الحالي: <?php echo basename($cours['fichier']); ?></span>
                                <a href="<?php echo $cours['fichier']; ?>" target="_blank">عرض</a>
                            </div>
                        <?php endif; ?>
                        <div class="form-file">
